 ...
 * @param {方法} func
 */
note.addLoadEvent = function (func) {
 ...
 */
note.addEvent = function (dom, eventName, func, useCapture) {
 ...
 */
note.addClass = function (dom, className) {
    if (!note.hasClass(dom, className)) {
        var c = dom.className || ''
        dom.className = c + ' ' + className
        dom.className = note.trim(dom.className)
    }
}
 ...
 * @param {class名} className
 */
note.hasClass = function (dom, className) {
 ...
 * @param {class名} className
 */
note.removeClass = function (dom, className) {
    if (note.hasClass(dom, className)) {
        var list = (dom.className || '').split(/\s+/)
        var newName = ''
        for (var i = 0; i < list.length; i++) {
        ...
        dom.className = note.trim(newName)
    }
}
 ...
 * @param {字符串} str
 */
note.encodeRegChar = function (str) {
    // \ 必须在第一位
    var arr = ['\\', '.', '^', '$', '*', '+', '?', '{', '}', '[', ']', '|', '(', ')']
 ...
 * @param {字符串} str
 */
note.ajax = function (option, success, fail) {
    var xmlHttp = null
...
note.addLoadEvent(function () {
    ...
    function ckeckToShow() {
        if (getScrollTop() > 200) {
            note.addClass(toTopDOM, 'show')
        } else {
            note.removeClass(toTopDOM, 'show')
        }
    }
    note.addEvent(window, 'scroll', ckeckToShow)
    note.addEvent(
        toTopDOM,
        'click',
        function (event) {
