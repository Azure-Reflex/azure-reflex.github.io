...
function loadAllPostData(callback) {
    if (localStorage.db && localStorage.dbVersion == note.buildAt) {
    ...
    note.ajax(
        {
            timeout: 20000,
            url: note.baseurl + '/static/xml/search.xml?t=' + note.buildAt
        },
        function (data) {
            localStorage.db = data
            localStorage.dbVersion = note.buildAt
...
note.addLoadEvent(function () {
    ...
    function search(key) {
        ...
        for (let i = 0; i < doms.length; i++) {
            ...
            let hide = true
            let r1 = new RegExp(note.encodeRegChar(key), 'gi')
            let r2 = new RegExp(note.encodeRegChar(key), 'i')
    ...
    note.addEvent(input, 'input', function (event) {
        if (!inputLock) {
            search(event.target.value)
        }
    })

    note.addEvent(input, 'compositionstart', function (event) {
        inputLock = true
    })

    note.addEvent(input, 'compositionend', function (event) {
        inputLock = false
        search(event.target.value)
    })
})
